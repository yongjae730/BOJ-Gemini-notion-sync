...
### ⚠️ 노션 데이터베이스 설정 필수 (중요!)

연동할 노션 데이터베이스는 반드시 아래 **5가지 속성(Property)** 을 가지고 있어야 합니다. 이름이 정확해야 작동합니다.

- `이름` (제목 속성) : 문제 제목이 들어갑니다.
- `날짜` (날짜 속성) : 풀이 날짜가 들어갑니다.
- `알고리즘` (다중 선택/Multi-select) : 문제의 태그가 들어갑니다.
- `난이도` (선택/Select) : Solved.ac 티어(예: Gold 5)가 들어갑니다.
- `언어` (선택/Select) : 사용한 언어(예: Java, Python)가 들어갑니다.
...
이 프로젝트는 크롬 웹 스토어에 등록된 정식 앱이 아니므로, **크롬 개발자 모드**를 통해 수동으로 설치해야 합니다.

### 1️⃣ 소스 코드 다운로드

1.  이 레포지토리 페이지 우측 상단의 초록색 **[<> Code]** 버튼을 클릭합니다.
2.  **[Download ZIP]** 을 눌러 압축 파일을 다운로드하고 **압축을 풉니다.**

### 2️⃣ 크롬 확장 프로그램 관리 페이지 접속

1.  크롬 브라우저 주소창에 `chrome://extensions`를 입력합니다.
2.  우측 상단의 **개발자 모드(Developer mode)** 스위치를 켭니다.

### 3️⃣ 확장 프로그램 로드

1.  좌측 상단의 **[압축해제된 확장 프로그램을 로드합니다] (Load unpacked)** 버튼을 클릭합니다.
2.  압축을 풀어둔 폴더(`BOJ-Extension`) 자체를 선택합니다.
...
만약 코드를 직접 수정했거나, 최신 버전으로 덮어씌운 경우에는 **반드시 새로고침**을 해야 적용됩니다.
